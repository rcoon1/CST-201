...
//this is the work of Ryan Coon along with code provided from the class
public class DLinkedList<T extends Comparable<T>> {
	 ...
	 *
	 * @param val
	 * @return true if successful, false otherwise **time complexity operation is O(1)
	 *         but overall is O(n)
	 */
	 ...
	 * @param item
	 */
	//O(n) time complexity items need to be placed in the right position, and some already are where they are needed. linear time
	...
	// Added<--O(n) time complexity. n is number of shifted elements
	public boolean insertMerge(T item) {
		DNode current = header.next;
		while (current != header && current.data.compareTo(item) < 0) {
			current = current.next;
		}
	...
	public DLinkedList merge(DLinkedList rhs) {
		...
		while (current != rhs.header) {
			result.insertMerge(current.data);
			current = current.next;
		}
		return result;
	}
